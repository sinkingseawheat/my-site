...
<>
      ...
      <S.lv2 title={`出力`}>
        <Suspense fallback={<div className={style.outputLoading} aria-busy="true">計算中です...</div>}>
          <div className={style.outputLoading} aria-busy="false">
            {
              output === null ?
              <>まだ1回も実行されてません、もしくは入力が無効です。</>
              : <div>{Array.from(Object.entries(output)).map(([p,v],index)=>(<span key={index}>{`${p}:${v}`}<br/></span>))}</div>
            }
          </div>
        </Suspense>
      </S.lv2>
